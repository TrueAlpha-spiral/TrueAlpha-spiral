...
class TrueAlphaSpiral:
    ...
    def initialize(self):
        ...
        print(f"  Quantum Echo Authentication: {'✓' if self.echo_authenticator else '✗'}")
        
        # Add Quantum Echo status if available
        if self.echo_authenticator:
            echo_status = self.echo_authenticator.get_status()
            channel_secure = echo_status.get("channel_secure", False)
            haiku_verified = echo_status.get("haiku_verified", False)
            echo_resonance = echo_status.get("echo_resonance", 0.0)
            
            print("\nQuantum Echo Authentication Status:")
            print(f"  Channel Secure: {'✓' if channel_secure else '✗'}")
            print(f"  Haiku Verification: {'✓' if haiku_verified else '✗'}")
            print(f"  Echo Resonance: {echo_resonance:.4f}")
            print(f"  Threat Level: {echo_status.get('threat_level', 0.0):.4f}")
        print("\nSystem State:")
