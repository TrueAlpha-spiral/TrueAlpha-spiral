"""Run the Codex automation workflow.

This helper contacts the OpenAI API once to request a bash script that
clones the TAS repository, sets up the environment, and runs a self-test.
The bash script is saved to ``run.sh`` so you can inspect it before or after
execution. Each command runs sequentially and the final audit log hash is
reported as JSON. No steps are hidden or obfuscated.
"""
import os
import subprocess
import json
...
# Initialize OpenAI client using the API key from the environment. The
# script exits early if the key is missing so nothing runs without your
# approval.
